...
class FeatureFlagsState extends Equatable {
  ...
  const FeatureFlagsState({required this.showSyncBanner, required this.enableBatchSync, required this.batchSize});

  FeatureFlagsState copyWith({bool? showSyncBanner, bool? enableBatchSync, int? batchSize}) => FeatureFlagsState(
...
class FeatureFlagsCubit extends Cubit<FeatureFlagsState> {
  ...
  FeatureFlagsCubit()
      : super(FeatureFlagsState(
        ...
        batchSize: (KeyValueService.get<int>(_kSyncBatchSize) ?? 20).clamp(5, 50),
      ));
