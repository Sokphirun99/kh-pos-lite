...
class StockAdjustmentsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: Scaffold(
        appBar: AppBar(title: Text(AppLocalizations.of(context).adjustStockTitle)),
        body: BlocBuilder<ProductsBloc, ProductsState>(
          builder: (context, state) {
            if (state.isLoading) return const Center(child: CircularProgressIndicator());
            if (state.error != null) return Center(child: Text(state.error!));
            final items = state.items;
            if (items.isEmpty) return Center(child: Text(AppLocalizations.of(context).noProducts));
            return ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                return ListTile(
                  title: Text(p.name),
                  subtitle: Text('${AppLocalizations.of(context).stockLabel}: ${p.stock} • SKU: ${p.sku}'),
                  trailing: Row(
                    ...
                    children: [
                      IconButton(
                        ...
                        onPressed: () {
                          ...
                          context.read<ProductsBloc>().add(ProductUpdated(p.copyWith(stock: newStock)));
                        },
                      ),
                      IconButton(
                        ...
                        onPressed: () async {
                          ...
                          final value = await showDialog<int>(
                            ...
                            builder: (ctx) => AlertDialog(
                              title: Text(AppLocalizations.of(context).setStock),
                              content: TextField(
                          ...
                          if (value != null) {
                            context.read<ProductsBloc>().add(ProductUpdated(p.copyWith(stock: value)));
                          }
                        },
                      ),
                    ],
