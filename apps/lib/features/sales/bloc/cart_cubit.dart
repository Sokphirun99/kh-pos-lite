...
class CartCubit extends Cubit<CartState> {
  ...
  void addProduct(Product p, {int qty = 1}) {
    ...
    if (idx >= 0) {
      ...
      final nextQty = allowOversell ? desired : (desired > p.stock ? p.stock : desired);
  ...
  void removeLine(String productId) {
    emit(state.copyWith(items: state.items.where((e) => e.product.id != productId).toList()));
  }
    ...
    final items = state.items
        .map((e) {
          if (e.product.id == productId) {
            final nextQty = allowOversell ? qty : (qty > e.product.stock ? e.product.stock : qty);
  ...
  void increment(String productId) {
    final it = state.items.firstWhere((e) => e.product.id == productId, orElse: () => throw StateError('Not found'));
